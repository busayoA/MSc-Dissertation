...
class HiddenGraphLayer():
    ...
    def chooseCell(self):
        if self.layerName == "lstmcell":
            return self.LSTMCell(self.neurons, self.activationFuntion, return_state=True)
        elif self.layerName == "rnncell":
            return self.RNNCell(self.neurons, self.activationFuntion, return_state=True)
        elif self.layerName == "grucell":
            return self.RNNCell(self.neurons, self.activationFuntion, return_state=True)

    def RNNCell(self, neurons: int, activationFunction: str):
        activationFunction = self.getActivationFunction(activationFunction)
        return tf.keras.layers.SimpleRNNCell(neurons, activation=activationFunction)

    def LSTMCell(self, neurons: int, activationFunction: str):
        activationFunction = self.getActivationFunction(activationFunction)
        return tf.keras.layers.LSTMCell(neurons, activation=activationFunction)
